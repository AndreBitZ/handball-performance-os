...
import type { Club, Competition, Match, MatchEvent, Player, PlayerTeamSeason, Playlist, Season, Team, Clip, MatchSquad, MatchPlayerInterval, MatchClockState, MatchBackupSnapshot, MatchVideoSync, MatchVideo } from './types';

export class HandballPerformanceDB extends Dexie {
  clubs!: EntityTable<Club, 'id'>; teams!: EntityTable<Team, 'id'>; seasons!: EntityTable<Season, 'id'>; players!: EntityTable<Player, 'id'>; playerTeamSeasons!: EntityTable<PlayerTeamSeason, 'id'>; competitions!: EntityTable<Competition, 'id'>; matches!: EntityTable<Match, 'id'>; matchSquads!: EntityTable<MatchSquad, 'id'>; playerIntervals!: EntityTable<MatchPlayerInterval, 'id'>; clockStates!: EntityTable<MatchClockState, 'id'>; videoSyncs!: EntityTable<MatchVideoSync, 'id'>; videos!: EntityTable<MatchVideo, 'id'>; backupSnapshots!: EntityTable<MatchBackupSnapshot, 'id'>; events!: EntityTable<MatchEvent, 'id'>; clips!: EntityTable<Clip, 'id'>; playlists!: EntityTable<Playlist, 'id'>;
  constructor() {
    ...
    this.version(7).stores({ videoSyncs: 'id, matchId, updatedAt' });
    this.version(8).stores({ videos: 'id, matchId, createdAt, updatedAt' });
  }
}
