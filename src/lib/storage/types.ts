...
export interface Match { id: UUID; seasonId: UUID; competitionId?: UUID; teamId: UUID; opponentName: string; date: string; venue?: string; homeAway: 'HOME' | 'AWAY' | 'NEUTRAL'; goalsFor?: number; goalsAgainst?: number; status: 'PLANNED' | 'IN_PROGRESS' | 'COMPLETED'; sourceVideoPath?: string; videoDurationSeconds?: number; createdAt: string; updatedAt: string; }
export interface MatchSquad { id: UUID; matchId: UUID; playerId: UUID; starter: boolean; captain: boolean; shirtNumber?: number; position?: Position; }
export interface MatchEvent { id: UUID; matchId: UUID; timestampSeconds: number; durationSeconds?: number; type: string; teamId?: UUID; playerId?: UUID; zone?: string; distance?: string; shotType?: string; result?: string; attackPhase?: string; notes?: string; tags?: string[]; createdAt: string; }
