...
function displayTeamName(team: Team | undefined, fallback: string) {
  return team?.name?.trim() || fallback
}

/**
 * Builds the package sent to Andebol-Stats before a live match.
 * The package contains only match setup data; LIVE events are created by Andebol-Stats.
 */
export async function buildLivePackage(matchId: string): Promise<LivePackage> {
  ...
  const [homeTeam, awayTeam, squads] = await Promise.all([
    db.teams.get(match.homeAway === 'AWAY' ? match.opponentTeamId ?? '' : match.teamId),
    db.teams.get(match.homeAway === 'AWAY' ? match.teamId : match.opponentTeamId ?? ''),
    db.matchSquads.where('matchId').equals(matchId).toArray(),
  ])

  const playerIds = [...new Set(squads.map((row) => row.playerId))]
  const players = (await Promise.all(playerIds.map((id) => db.players.get(id)))).filter(Boolean) as Player[]
  ...
  return {
    ...
    match: {
      ...
      homeTeamId,
      awayTeamId,
      homeTeamName: displayTeamName(homeTeam, match.homeAway === 'AWAY' ? match.opponentName : 'Casa'),
      awayTeamName: displayTeamName(awayTeam, match.homeAway === 'AWAY' ? 'Fora' : match.opponentName),
    ...
    players: players.map((player) => {
      const squad = squads.find((row) => row.playerId === player.id)
      return {
        id: player.id,
        name: player.displayName,
        shirtNumber: squad?.shirtNumber ?? player.shirtNumber,
        position: squad?.position ?? player.position,
        teamId: squad?.teamId ?? match.teamId,
        active: player.active,
      }
    }),
    roster: squads.map((row: MatchSquad) => ({
      id: row.id,
      playerId: row.playerId,
      teamId: row.teamId ?? match.teamId,
...
export async function downloadLivePackage(matchId: string) {
  ...
  anchor.href = url
  anchor.download = `live-package-${payload.match.id}.json`
  anchor.click()
