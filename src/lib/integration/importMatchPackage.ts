...
export function validateImportedMatchPackage(payload: unknown): payload is CanonicalMatchPackage {
  ...
  return !!value && typeof value === 'object' && value.schemaVersion === '1.1.0' && !!value.match?.id && Array.isArray(value.players) && Array.isArray(value.roster) && Array.isArray(value.events);
}
...
export async function importAndebolStatsResult(database: HandballPerformanceDB, payload: CanonicalMatchPackage) {
  ...
  await database.transaction('rw', database.matches, database.matchSquads, database.events, async () => {
    await database.matches.update(payload.match.id, { goalsFor: payload.match.homeScore ?? existingMatch.goalsFor, goalsAgainst: payload.match.awayScore ?? existingMatch.goalsAgainst, status: payload.match.status === 'finished' ? 'COMPLETED' : existingMatch.status, updatedAt: now });
    ...
    for (const roster of payload.roster) {
      ...
      if (existingSquadIds.has(roster.id)) await database.matchSquads.update(roster.id, { starter: roster.starter, shirtNumber: roster.shirtNumber ?? undefined, position });
      else await database.matchSquads.add({ id: roster.id, matchId: payload.match.id, playerId: roster.playerId, teamId: roster.teamId, starter: roster.starter, captain: false, shirtNumber: roster.shirtNumber ?? undefined, position });
    }
    ...
    for (const event of payload.events) {
      ...
      const record = { id: event.id, matchId: payload.match.id, timestampSeconds: Number(event.gameTime), period: event.period, type: event.type, teamId: event.teamId ?? undefined, playerId: event.playerId ?? undefined, position: shot?.position as never, zone: shot?.zone ?? undefined, distance: shot?.distance ?? undefined, shotType: shot?.type ?? undefined, result: shot?.outcome ?? undefined, xg: shot?.xg ?? undefined, createdAt: now, updatedAt: now };
