...
import type { Match, MatchEvent, MatchSquad, Player, Team } from '../storage/types'

export const LIVE_PACKAGE_SCHEMA = '1.0.0'
...
export interface LivePackage {
  ...
  events: Array<{
    id: string; matchId: string; period?: 1 | 2; gameTime: number; teamId?: string; playerId?: string
...
function eventType(event: MatchEvent) {
  ...
  if (['shot', 'goal', 'miss', 'saved', 'blocked'].includes(type)) return 'shot'
  return type
}
...
function shotMetadata(event: MatchEvent) {
  ...
  return {
    shot: {
      ...
      outcome: event.result ?? null,
      attackPhase: event.attackPhase ?? null,
    },
  }
}
...
export async function buildLivePackage(matchId: string): Promise<LivePackage> {
  ...
  const [homeTeam, awayTeam, players, roster, events] = await Promise.all([
    ...
    match.opponentTeamId ? db.teams.get(match.opponentTeamId) : undefined,
    db.players.toArray(),
    db.matchSquads.where('matchId').equals(matchId).toArray(),
    db.events.where('matchId').equals(matchId).sortBy('timestampSeconds'),
  ])

  if (!homeTeam) throw new Error('Equipa do jogo não encontrada.')
  const awayTeamId = match.opponentTeamId ?? `opponent:${matchId}`
  const awayTeamName = awayTeam?.name ?? match.opponentName
  const matchPlayers = new Map(roster.map(r => [r.playerId, r]))
  const selectedPlayers = players.filter(p => matchPlayers.has(p.id))

  return {
    schemaVersion: LIVE_PACKAGE_SCHEMA,
    source: 'handball-performance-os',
    match: {
      id: match.id,
      ...
      durationMinutes: 30,
      currentPeriod: events.some(e => e.period === 2) ? 2 : 1,
      gameTime: Math.max(...events.map(e => e.timestampSeconds), 0),
      homeTeamId: homeTeam.id,
      awayTeamId,
      homeTeamName: homeTeam.name,
      awayTeamName,
      homeScore: match.goalsFor ?? 0,
      awayScore: match.goalsAgainst ?? 0,
    },
    teams: {
      home: { id: homeTeam.id, name: homeTeam.name },
      away: { id: awayTeamId, name: awayTeamName },
    },
    players: selectedPlayers.map((p: Player) => ({
      id: p.id,
      name: p.displayName,
    ...
    roster: roster.map((r: MatchSquad) => ({
      id: r.id,
      playerId: r.playerId,
      teamId: r.teamId ?? homeTeam.id,
    ...
    events: events.map((e: MatchEvent) => ({
      ...
      matchId: e.matchId,
      period: e.period,
      gameTime: e.timestampSeconds,
      teamId: e.teamId,
      playerId: e.playerId,
      type: eventType(e),
      metadata: { ...shotMetadata(e), notes: e.notes ?? null, tags: e.tags ?? [] },
    })),
    metadata: { exportedAt: new Date().toISOString(), sourceVersion: 'performance-os-1.0' },
  }
}
...
export async function exportLivePackage(matchId: string) {
  ...
  link.href = url
  link.download = `live-package-${payload.match.homeTeamName}-vs-${payload.match.awayTeamName}-${matchId}.json`
  link.click()
