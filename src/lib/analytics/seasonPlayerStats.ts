...
import { derivePlayerMinutes } from './playerMinutes'
...
function isFor(event: MatchEvent) { return event.result === 'FOR' || event.teamId !== undefined }

export function deriveSeasonPlayerStats(seasonId: string, teamId: string, players: Player[], relations: PlayerTeamSeason[], matches: Match[], squads: MatchSquad[], events: MatchEvent[]): SeasonPlayerStats[] {
  ...
  for (const player of players) if (relationIds.has(player.id)) stats.set(player.id, { playerId: player.id, name: player.displayName, matches: 0, starts: 0, minutes: 0, goals: 0, shots: 0, missedShots: 0, efficiency: 0, assists: 0, turnovers: 0, steals: 0, exclusions2m: 0, goalsPerMatch: 0, shotsPerMatch: 0 })

  for (const match of seasonMatches) {
    const matchSquad = squads.filter(s => s.matchId === match.id)
    for (const squad of matchSquad) {
      const row = stats.get(squad.playerId)
      if (!row) continue
      row.matches++
      if (squad.starter) row.starts++
      row.minutes += derivePlayerMinutes(events.filter(e => e.matchId === match.id), matchSquad, squad.playerId)
    }
