...
type MemoryEntry = { kind: 'file'; content: string } | { kind: 'directory'; entries: Map<string, MemoryEntry> }

function directory(entries = new Map<string, MemoryEntry>()): FileSystemDirectoryHandle {
  return {
    kind: 'directory', name: 'memory',
    async getDirectoryHandle(name: string, options?: FileSystemGetDirectoryOptions) {
      const entry = entries.get(name)
      if (entry?.kind === 'directory') return directory(entry.entries)
      if (options?.create) { const created: MemoryEntry = { kind: 'directory', entries: new Map() }; entries.set(name, created); return directory(created.entries) }
      throw new DOMException(`Directory ${name} not found`, 'NotFoundError')
    },
    async getFileHandle(name: string, options?: FileSystemGetFileOptions) {
      const entry = entries.get(name)
      if (entry?.kind === 'file') return file(name, entry)
      if (options?.create) { const created: MemoryEntry = { kind: 'file', content: '' }; entries.set(name, created); return file(name, created) }
...
function file(name: string, entry: { kind: 'file'; content: string }): FileSystemFileHandle {
  return {
    ...
    async createWritable() {
      ...
      return { async write(data: FileSystemWriteChunkType) { next = typeof data === 'string' ? data : await new Response(data as BodyInit).text() }, async close() { entry.content = next }, async abort() {}, locked: false } as FileSystemWritableFileStream
    },
