...
import { buildLivePackage, LIVE_PACKAGE_SCHEMA } from '../../src/lib/integration/live-package'

const now = new Date().toISOString()

describe('Performance OS → Andebol-Stats live package', () => {
  it('exports the canonical package expected by the Andebol-Stats importer', async () => {
    ...
    await database.events.add({ id:'e1', matchId:'m1', timestampSeconds:120, period:1, type:'goal', teamId:'t1', playerId:'p1', zone:'Z3', distance:'6-9m', shotType:'jump', result:'goal', attackPhase:'positional', createdAt:now })
    ...
    expect(pkg.roster[0]).toMatchObject({ playerId:'p1', starter:true, captain:true })
    expect(pkg.events[0]).toMatchObject({ id:'e1', type:'shot', playerId:'p1', gameTime:120 })
    expect(pkg.events[0].metadata).toMatchObject({ shot:{ shooterId:'p1', zone:'Z3', distance:'6-9m', type:'jump', outcome:'goal', attackPhase:'positional' } })
