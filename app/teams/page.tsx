...
export default function TeamsPage() {
  ...
  const [editGender, setEditGender] = useState<Team['gender']>('F')
  const [editClubId, setEditClubId] = useState('')
  const [editClubName, setEditClubName] = useState('')

  async function load() { if (!db) return; const [nextTeams, nextClubs] = await Promise.all([db.teams.orderBy('name').toArray(), db.clubs.orderBy('name').toArray()]); setTeams(nextTeams); setClubs(nextClubs); setClubId(current => current || nextClubs[0]?.id || '') }
  useEffect(() => { void load() }, [])
  ...
  async function removeTeam(id: string) { if (!db || !window.confirm('Apagar esta equipa?')) return; await db.teams.delete(id); await load() }
  ...
  return <main className="content standalonePage">
    ...
    <section className="section">
      ...
      <form onSubmit={addClub} className="localForm"><input value={clubName} onChange={e => setClubName(e.target.value)} placeholder="Nome do clube" aria-label="Nome do clube" /><button type="submit"><Plus size={17}/> Criar clube</button></form>
      {clubs.length > 0 && <div className="localList" style={{ marginTop: 12 }}>{clubs.map(club => editingClubId === club.id ? <div className="localRow" key={club.id}><input value={editClubName} onChange={e => setEditClubName(e.target.value)} /><button className="iconButton" onClick={() => void saveClubEdit()} aria-label="Guardar clube"><Check size={17}/></button><button className="iconButton" onClick={() => setEditingClubId(null)} aria-label="Cancelar"><X size={17}/></button></div> : <div className="localRow" key={club.id}><span style={{ flex: 1 }}><strong>{club.name}</strong><small style={{ display: 'block', opacity: .7 }}>Clube</small></span><button className="iconButton" onClick={() => beginClubEdit(club)} aria-label={`Editar ${club.name}`}><Pencil size={17}/></button><button className="iconButton" onClick={() => void removeClub(club.id)} aria-label={`Apagar ${club.name}`}><Trash2 size={17}/></button></div>)}</div>}
      <form onSubmit={addTeam} className="localForm"><select value={clubId} onChange={e => setClubId(e.target.value)} aria-label="Clube"><option value="">Selecionar clube</option>{clubs.map(club => <option key={club.id} value={club.id}>{club.name}</option>)}</select><input value={name} onChange={e => setName(e.target.value)} placeholder="Nome da equipa" required /><input value={category} onChange={e => setCategory(e.target.value)} placeholder="Categoria" /><select value={gender} onChange={e => setGender(e.target.value as Team['gender'])}><option value="F">Feminino</option><option value="M">Masculino</option><option value="MIXED">Misto</option></select><button type="submit"><Plus size={17}/> Criar equipa</button></form>
      ...
      <div className="localList">
        ...
        {teams.map(team => editingTeamId === team.id ? <div className="localRow" key={team.id} style={{ flexWrap: 'wrap' }}><input value={editName} onChange={e => setEditName(e.target.value)} aria-label="Nome da equipa" /><input value={editCategory} onChange={e => setEditCategory(e.target.value)} aria-label="Categoria" /><select value={editGender} onChange={e => setEditGender(e.target.value as Team['gender'])}><option value="F">Feminino</option><option value="M">Masculino</option><option value="MIXED">Misto</option></select><select value={editClubId} onChange={e => setEditClubId(e.target.value)} aria-label="Clube">{clubs.map(club => <option key={club.id} value={club.id}>{club.name}</option>)}</select><button className="iconButton" onClick={() => void saveTeamEdit()} aria-label="Guardar equipa"><Check size={17}/></button><button className="iconButton" onClick={() => setEditingTeamId(null)} aria-label="Cancelar"><X size={17}/></button></div> : <div className="localRow" key={team.id}><Link href={`/teams/${team.id}`} style={{ flex: 1 }}><strong>{team.name}</strong><span>{clubNames.get(team.clubId) ?? 'Clube desconhecido'} · {team.category} · {team.gender === 'F' ? 'Feminino' : team.gender === 'M' ? 'Masculino' : 'Misto'}</span></Link><button className="iconButton" onClick={() => beginTeamEdit(team)} aria-label={`Editar ${team.name}`}><Pencil size={17}/></button><button className="iconButton" onClick={() => void removeTeam(team.id)} aria-label={`Apagar ${team.name}`}><Trash2 size={17}/></button></div>)}
      </div>
    </section>
  </main>
}
