...
const nav = [
  ['Dashboard', '/'], ['Equipa', '/teams'], ['Jogadores', '/players'], ['Épocas', '/seasons'], ['Jogos', '/games'],
  ['Video Analysis', '#'], ['Performance', '#'], ['Scouting', '#'], ['Treinos', '#'], ['Reports', '#'],
]
...
export default function Home() {
  ...
  return <main className="shell">
    <aside className="sidebar">
      ...
      <nav className="nav">{nav.map(([item, href], index) => <a className={index === 0 ? 'navItem active' : 'navItem'} href={href} key={item}><span>{['⌂','◆','●','◷','▣','▶','◈','⌕','▤','▥'][index]}</span>{item}</a>)}</nav>
