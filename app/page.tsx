    ...
    <main className="shell">
      ...
      <section className="content">
        <header className="topbar">
          <div>
            <p className="eyebrow">SPORT PERFORMANCE</p>
            <h1>Dashboard</h1>
          </div>
          <div className="topActions">
            <button className="search"><Search size={17} /> Pesquisar</button>
        ...
        <div className="hero">
          <div>
            <p className="eyebrow">ÉPOCA 2026/27</p>
            <h2>Vamos construir o teu centro de performance.</h2>
            <p>Uma plataforma única para análise de vídeo, scouting, estatística e gestão da performance.</p>
          </div>
        ...
        <section className="section">
          <div className="sectionHeader">
            <div><p className="eyebrow">ARQUITETURA</p><h3>Módulos principais</h3></div>
            <span className="status">Fundação</span>
          </div>
          <div className="moduleGrid">
            {modules.map(([emoji, title, description]) => (
              <article className="moduleCard" key={title}>
                <div className="moduleEmoji">{emoji}</div>
                <h4>{title}</h4>
                <p>{description}</p>
                <span>Em breve →</span>
              </article>
        ...
        <section className="section roadmap">
          ...
          <div className="steps">
            {['Fundação', 'Gestão', 'Video Engine', 'Coding & Clips', 'Telestration', 'Performance & xG', 'Scouting', 'AI'].map((step, i) => (
              <div className={i === 0 ? 'step current' : 'step'} key={step}><b>{i + 1}</b><span>{step}</span></div>
            ))}
          </div>
        </section>
      </section>
