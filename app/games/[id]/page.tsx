...
import { downloadLivePackage } from '../../../src/lib/match/live-package'
...
export default function GameDetailPage({ params }: { params: { id: string } }) {
  ...
  async function exportForStats() {
    ...
    try { await downloadLivePackage(currentMatch.id); setMessage('Pacote LIVE criado. Importa este ficheiro no Andebol-Stats.') }
  ...
  return <main className="content standalonePage">
    ...
    <section className="hero"><div><p className="eyebrow">{competition?.name ?? 'Sem competição'}</p><h2>{team?.name ?? 'Equipa'} <span style={{ opacity:.65 }}>vs</span> {currentMatch.opponentName}</h2><p>{season?.name ?? 'Época'} · {currentMatch.status === 'PLANNED' ? 'Jogo planeado' : currentMatch.status === 'IN_PROGRESS' ? 'Em curso' : 'Concluído'}</p></div><div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:10}}><div className="heroBadge">{currentMatch.goalsFor ?? '-'} : {currentMatch.goalsAgainst ?? '-'}</div><button onClick={()=>void exportForStats()} disabled={exporting}><Download size={16}/>{exporting?'A preparar…':'Exportar para Andebol-Stats'}</button></div></section>
    {message && <section className="section"><p role="status">{message}</p></section>}
    <section className="moduleGrid"><Link href={`/games/${currentMatch.id}/squad`} className="moduleCard"><div className="moduleEmoji"><Users/></div><h4>Convocados</h4><p>Selecionar os jogadores disponíveis para este jogo.</p><span>ABRIR →</span></Link><Link href={`/games/${currentMatch.id}/live`} className="moduleCard"><div className="moduleEmoji"><Play/></div><h4>Live Match</h4><p>Iniciar cronómetro e registar eventos em direto.</p><span>ABRIR →</span></Link><div className="moduleCard"><div className="moduleEmoji"><BarChart3/></div><h4>Estatísticas</h4><p>Análise do jogo, jogadores e indicadores de performance.</p><span>PRÓXIMO MÓDULO</span></div><div className="moduleCard"><div className="moduleEmoji"><Video/></div><h4>Vídeo</h4><p>Vídeo, eventos e clips associados ao jogo.</p><span>PRÓXIMO MÓDULO</span></div></section>
  </main>
}