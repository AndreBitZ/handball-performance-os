...
function calculate(events: MatchEvent[], players: Player[]): PlayerMatchStats[] {
  return players.map(player => {
    ...
    return {
      ...
      steals,
      exclusions,
      shootingEfficiency: shots ? Math.round((goals / shots) * 100) : 0,
    }
  })
}
...
export default function MatchStats({ matchId, players }: { matchId: string; players: Player[] }) {
  ...
  return <section className="section"><div className="sectionHeader"><div><p className="eyebrow">ANÁLISE</p><h2>Estatísticas do jogo</h2><p>Calculadas diretamente a partir dos eventos registados no Live Match.</p></div></div><div className="localList"><article className="localRow"><strong>Equipa</strong><span>{totals.goals} golos · {totals.shots} remates · {totals.shots ? Math.round((totals.goals / totals.shots) * 100) : 0}% eficácia · {totals.assists} assistências · {totals.turnovers} perdas · {totals.steals} recuperações · {totals.exclusions} exclusões</span></article>{stats.filter(row => row.shots || row.goals || row.assists || row.turnovers || row.steals || row.exclusions).map(row => { const player = players.find(p => p.id === row.playerId); if (!player) return null; return <article className="localRow" key={row.playerId}><strong>{player.displayName}</strong><span>{row.goals}G · {row.shots}R · {row.shootingEfficiency}% · {row.assists}A · {row.turnovers}P · {row.steals}Rec · {row.exclusions}×2'</span></article> })}</div></section>
}
