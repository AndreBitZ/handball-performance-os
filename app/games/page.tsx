...
export default function GamesPage() {
  ...
  const seasonCompetitions = useMemo(
    () => competitions.filter(c => !seasonId || c.seasonId === seasonId),
  ...
  async function remove(id: string) {
    if (db && window.confirm('Apagar este jogo?')) {
  ...
  return <main className="content standalonePage">
    ...
    <section className="section">
      <form onSubmit={add} className="localForm">
        <select value={teamId} onChange={e => setTeamId(e.target.value)} required aria-label="Equipa">
          <option value="">Equipa</option>
          {teams.map(team => <option key={team.id} value={team.id}>{team.name}</option>)}
        </select>
        <select value={seasonId} onChange={e => { setSeasonId(e.target.value); setCompetitionId('') }} required aria-label="Época">
          <option value="">Época</option>
          {seasons.map(season => <option key={season.id} value={season.id}>{season.name}</option>)}
        </select>
        <select value={competitionId} onChange={e => setCompetitionId(e.target.value)} aria-label="Competição">
          <option value="">Competição (opcional)</option>
          {seasonCompetitions.map(competition => <option key={competition.id} value={competition.id}>{competition.name}</option>)}
        </select>
        ...
        <select value={homeAway} onChange={e => setHomeAway(e.target.value as Match['homeAway'])} aria-label="Casa ou fora">
