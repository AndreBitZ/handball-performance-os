...
export default function NewGamePage() {
  ...
  useEffect(() => {
    ...
    ]).then(([t, s, c]) => { setTeams(t); setSeasons(s); setCompetitions(c); if (s[0]) setSeasonId(s[0].id) })
  }, [])
  ...
  async function submit(event: FormEvent) {
    ...
    const id = createId()
    await db.matches.add({ id, seasonId, competitionId: competitionId || undefined, teamId, opponentTeamId, opponentName: teams.find(t => t.id === opponentTeamId)?.name ?? 'Equipa', date, venue: venue.trim() || undefined, homeAway, status: 'PLANNED', createdAt: now, updatedAt: now })
